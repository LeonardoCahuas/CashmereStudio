...
const FonicoCalendar2 = ({ fonico, isAdmin }) => {
    ...
    useEffect(() => {
        if (selectedFonico) {
            ...
            if (fonico) {
                ...
                const disponibilita = fonico.disponibilita ? fonico.disponibilita.map(slot => convertSlotToDateTime(slot)) : [];
    ...
    const handleConferma = () => {
        ...
        if (editingDisponibilita) {
            ...
            const newDisp = fonici.find(f => f.id == selectedFonico)?.disponibilita?.filter(elemento => !disponibilitaNumeri.includes(elemento))
    ...
    const renderCalendar = () => {
        ...
        const daysCode = ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];
...
export default FonicoCalendar2;

...
export function NavbarAdmin({ setCont, isAdmin }) {
    ...
    const renderDesktopNavbar = () => (
        <BootstrapNavbar expand="lg" bg="black" variant="dark" fixed="top" style={{ padding: "0px" }}>
            <Container style={{ margin: "0px", width: "100vw", paddingTop: "15px", paddingBottom: "15px", display:"flex", flexDirection:"row", justifyContent:"space-between" }}>
                ...
                <BootstrapNavbar.Collapse id="basic-navbar-nav">
                    <Nav className="m-auto d-flex flex-row align-items-center justify-content-end" style={{  gap: "60px" }} >
                        ...
                        {isAdmin && <Link className="nav-link mx-3" onClick={() => handleItemClick(2)}>Conferma</Link>}
                        {isAdmin && <Link className="nav-link mx-3" onClick={() => handleItemClick(3)}>Prenotazioni</Link>}
        ...
        <div className="w-100 d-flex flex-row align-items-center justify-content-between text-white navbar-container bg-black p-3 position-fixed m-0" style={{ paddingTop: "0px", zIndex: "999", top: "0px" }}>
            ...
            <div className='w-50'>
                <Link className={`navlogo w-100 d-flex flex-column ${'align-items-end'}`} style={{ paddingRight: "20px" }}>
                    <img src={logo} style={{ width: "25%", maxWidth:"50px" }} alt="Logo Cashmere Studio" />
                </Link>
            </div>
            <div className="collapse navbar-collapse navbar-collapse-fullscreen" id="navbarNavDropdown">
...

                {
                    isAdmin ?
                        ...
                        (
                            <ul className="navbar-nav">
                                {["Situazione", "Calendario", "Fonici"].map((label, index) => (
                                    <li className="w-100 nav-item d-flex flex-column align-items-start" style={{ paddingLeft: "30px" }} key={index}>
                                        <Link className="close nav-link" data-toggle="collapse" data-target="#navbarNavDropdown" style={{ fontSize: "25px", fontWeight: 900, color: "white", textDecoration: "none" }} onClick={() => index != 2 ? handleItemClick(index) : handleItemClick(4) }>{label}</Link>
                                    </li>
                                ))}
                            </ul>
                        )
